...
config.read('cutepihole.ini')

#weather config
varApiKey            = config['weather']['owm_api_key']
varFixedLocation     = config['weather']['fixed_location']
varLocation          = config['weather']['location']
#varInterval          = config['weather'].getint('interval')
varInterval          = 0

#pihole config
varPiholeApiUrl      = config['pihole']['pihole_api_url']
varPiholeApiPass     = config['pihole']['pihole_api_pass']
varPiholeDisableTime = config['pihole']['pihole_disable_time']
varPiholeConfigFile  = config['pihole']['pihole_config_file']
#panels config
varDefaultPanel      = config['panels']['default_panel']
if varDefaultPanel == "pihole":
    screenid = 1
elif varDefaultPanel == "weather":
    screenid = 2
elif varDefaultPanel == "stat":
    screenid = 3

#app config
varUpdatePanel       = config['app']['update_panel']
varDebug             = config['app']['debug']



# url to disable pihole
disable_url = varPiholeApiUrl + "?disable=" + varPiholeDisableTime + "&auth=" + varPiholeApiPass
...
def getWeather():
    ...
    url = "https://api.openweathermap.org/data/2.5/onecall?lat=%s&lon=%s&appid=%s&units=metric" % (lat, lon, varApiKey)
...
#rotation = 90 # need to figure this out
...
while True:
    ...
    # Pi Hole data!
    try:
        r = requests.get(varPiholeApiUrl)
